...
async function postView(info) {
    return updateOrInsertNotificationHistory(info, "viewed_on")
}

function postClick(info) {
    return updateOrInsertNotificationHistory(info, "clicked_on");
}

async function updateOrInsertNotificationHistory(info, updateField) {
    try {
        ...
        const doc = {};
        doc[updateField] = true;
        const update = {$currentDate: doc};
        ...
        */
        return await NotificationClickViewHistory.findOneAndUpdate(
            filter,
            update,
